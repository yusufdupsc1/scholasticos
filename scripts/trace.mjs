...
function parseCookies(cookieStr) {
    return cookieStr.split(";").flatMap((pair) => {
        ...
        return [{ name: name.trim(), value: value.trim(), domain: "localhost", path: "/", secure: true }];
    });
}
...
(async () => {
    ...
    if (HAMBURGER_SEL) {
        ...
        console.warn("  ⚠  hamburger: NOT IMPLEMENTED — skipped (see perf-baseline.md)");
    }

    // Sidebar nav interactions
    for (const sel of SIDEBAR_NAV_ITEMS) {
        const label = sel.split('href="')[1]?.replace('"]}', '"]').replace('"]', "");
