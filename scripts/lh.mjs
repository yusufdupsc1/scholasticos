...
async function runPage(chrome, { path: pagePath, slug }) {
    ...
    await fs.writeFile(jsonPath, typeof json === "string" ? json : JSON.stringify(lhr, null, 2), "utf8");
    ...
    console.log(`  ✓ score: ${score}  LCP: ${lcp}  TBT: ${tbt}  TTFB: ${ttfb}  CLS: ${cls}  FCP: ${fcp}  SI: ${si}  JS: ${(jsBytes / 1024).toFixed(0)}KB`);
    console.log(`     html → ${htmlPath}`);
    console.log(`     json → ${jsonPath}`);

    return { slug, score, fcp, lcp, tbt, ttfb, cls, si, jsBytes, htmlPath, jsonPath };
}
